...
public class PersonalGoalCard3 extends PersonalGoalCard{
    public PersonalGoalCard3(){
        ...
        objective[1][0] = ItemType.FRAME;
        objective[1][3] = ItemType.GAME;
        objective[2][2] = ItemType.PLANT;
        objective[3][1] = ItemType.CAT;
        objective[3][4] = ItemType.TROPHY;
        objective[5][0] = ItemType.BOOK;
    }
    @Override
    public int calculateScore(ItemTile[][] matrix) {
        int score=0;

        if(matrix[1][0].getType() == objective[1][0]){ score = score + 1;}
